...
export default function BookingCalendar() {
  ...
  useEffect(() => {
    fetch('/api/wix/services')
      ...
      .then(data => {
        if (data?.items?.length) setServices(data.items.map(mapWixService));
      })
      .catch(() => {});
  }, []);
            ...
            {services.map(service => (
              ...
              >
                <div>
                  ...
                  <p className="muted" style={{ fontSize: '0.875rem', marginBottom: service.bullets?.length ? 'var(--space-sm)' : 0 }}>
                    {service.description}
                  </p>
          ...
          <div className="card" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 'var(--space-lg)', alignItems: 'center', flexWrap: 'wrap' }}>
            <div>
              ...
              <p className="muted" style={{ fontSize: '0.875rem' }}>
                Send a message and we&apos;ll figure it out together. No pitch, no pressure.
              </p>
            </div>
