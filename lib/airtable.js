...
function writingReady() {
  const key = process.env.AIRTABLE_API_KEY || '';
  const baseId = process.env.AIRTABLE_WRITING_BASE_ID || '';
  if (!key || !baseId) return false;
  const placeholder = /your_airtable|pat_xxxxx|changeme|placeholder/i.test(key);
  return !placeholder;
}

function isAirtableAuthError(err) {
...
export async function getProducts({ liveOnly = false } = {}) {
  ...
  return records.map(r => ({
    ...
    gumroadUrl:    r.fields['Gumroad URL']         ?? r.fields['Product URL'] ?? '',
...
export async function getContacts({ status } = {}) {
  ...
  return records.map(r => ({
    ...
    isMagnolia:     r.fields['Magnolia Circle']    ?? false,
  }));
}
...
export async function getResumes() {
  ...
  try {
    ...
    if (isAirtableAuthError(err)) {
      console.warn('[airtable] resumes unavailable:', err.message);
      return null;
    }
    throw err;
  }
...
export async function getOpportunities({ status, track } = {}) {
  ...
  try {
    ...
    if (isAirtableAuthError(err)) {
      console.warn('[airtable] opportunities unavailable:', err.message);
      return null;
    }
    throw err;
  }
...
export async function getMlisPrograms() {
  ...
  if (!baseId || !process.env.AIRTABLE_API_KEY) return null;
  try {
    ...
    if (isAirtableAuthError(err)) {
      console.warn('[airtable] mlis unavailable:', err.message);
      return null;
    }
    throw err;
  }
