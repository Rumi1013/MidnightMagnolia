...
const TBL = {
  ...
  RESUMES:            () => process.env.AIRTABLE_TBL_RESUMES             || 'tblkPLNijXQMQLETB',
  OPPORTUNITIES:      () => process.env.AIRTABLE_TBL_OPPORTUNITIES       || 'tblTh9CIR4mypfBMT',
...
function createRecord(baseRef, tableId, fields) {
  return baseRef()(tableId).create(fields);
}
...
export async function createContact(fields) {
  return createRecord(opsBase, TBL.CONTACTS(), fields);
}

export async function createSalesLogEntry(fields) {
  return createRecord(opsBase, TBL.SALES(), fields);
}
