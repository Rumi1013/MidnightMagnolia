...
| `site-nav-shop` | Shop → Stan | **Done:** `NAV` uses `URLS.stanStore` with `external: true` | Remove duplicate Shop row in Wix menus if any |
...
| `site-grimoire-pg` | Grimoire gate | `/grimoire` + Stan/BMAC CTAs | — |
...
**Memberships / support (canonical):** Stan + Buy Me a Coffee first; Patreon only when access is restored — reflected in `Layout` footer and Master Plan HTML row for BMAC.
