...
- For ASAP public ship, prioritize the marketing site (including a real `/portfolio` with real projects + resume links), BMAC-first digital/membership commerce, and a minimal operable content→BMAC product pipeline; treat Career Command as a private ops panel first.
- Digital/membership checkout should lead with **Buy Me a Coffee** (`NEXT_PUBLIC_BMAC_URL` / `URLS.bmac`); Stan Store is deferred (cost). Keep Wix Headless for catalog/bookings/optional physical SKUs rather than adding Stripe/Shopify for v1.
- Prefer finishing env, BMAC QA, portfolio content, and one digital content→BMAC listing before apex DNS cutover from classic Wix publish to Vercel. Dropship (Printify) is an explicit post-ASAP lane — do not forget, do not block digital ship.
...
- Wix Headless OAuth app `Midnitemag` (`977ae287-172d-4cd1-9a46-42bcc3b5b863`) on site `ad2ce561-4efa-4255-a998-9074ffc0de7b` has Vercel hosts in `allowedRedirectDomains`; `allowedRedirectUris` is still empty — member login needs exact `/auth/callback` URI(s).
- Nav “The Shop” and digital CTAs use Buy Me a Coffee (`https://www.buymeacoffee.com/midnightmagnolia` via `NEXT_PUBLIC_BMAC_URL` / `URLS.bmac`); Stan Store URL remains in constants for reference only (deferred). `/shop` remains Wix Stores catalog via Headless. Ship gate: `docs/BMAC_MINIMUM.md` + `docs/DIGITAL_CONTENT_PIPELINE.md`. Dropship: `docs/DROPSHIP_LANE.md`.
...
- Ship execution index: `docs/implementation-sources.md` — prefer BMAC_MINIMUM, checkout loop (BMAC-first), site integrity, Wix merch, RLS; hermes Gumroad-primary is not the Active ASAP gate.
